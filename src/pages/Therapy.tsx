 ...
 * 
 * CHANGES IMPLEMENTED:
 * 1. Internal Message Cleanup: Added cleanInternalMarkers() function to remove ** ... ** markers
 *    - Applied to all AI responses before displaying to users
 *    - Ensures system/internal messages never appear in chat UI
 * 
 * 2. Persistent Chat History: Enhanced fetchSessionAndMessages() to load complete conversation history
 *    - Added /api/messages endpoint to fetch all messages for a session
 *    - Modified session API to return sessionId for message fetching
 *    - Chat history now persists across page refresh and login/logout
 * 
 * 3. Message Cleaning Integration: Applied cleanInternalMarkers() to:
 *    - First messages from session_first_message
 *    - AI responses in handleSendMessage()
 *    - Messages loaded from persistent chat history
 * 
 * These changes ensure that therapy sessions maintain their full conversation history
 * while keeping the UI clean and free of internal system markers.
 */
...
// Utility to clean internal/system markers from AI responses
// This ensures that ** ... ** markers never appear in user-facing content
function cleanInternalMarkers(text: string): string {
  if (!text) return text;
  
  // Check if there are internal markers to clean
  const hasInternalMarkers = /\*\*[^*]*\*\*/.test(text);
  
  if (hasInternalMarkers) {
    console.log('🧹 Cleaning internal markers from AI response:', text.substring(0, 100) + '...');
  }
  
  // Remove internal markers wrapped in ** ... **
  // These are system/internal messages that should never be shown to users
  let cleaned = text.replace(/\*\*[^*]*\*\*/g, '');
  
  // Remove any double spaces or extra whitespace that might result from cleaning
  cleaned = cleaned.replace(/\s+/g, ' ').trim();
  
  // If cleaning resulted in empty content, provide a fallback
  if (!cleaned || cleaned.length < 10) {
    console.log('⚠️ Internal marker cleaning resulted in empty content, using fallback');
    cleaned = "I'm here to support you. How are you feeling right now?";
  }
  
  if (hasInternalMarkers) {
    console.log('✅ Internal markers cleaned, result:', cleaned.substring(0, 100) + '...');
  }
...
function highlightTherapyQuestion(text: string): JSX.Element {
  // Clean internal markers first before processing
  const cleanedText = cleanInternalMarkers(text);
  
  // Find the last question mark
  const lastQ = cleanedText.lastIndexOf('?');
  if (lastQ === -1) return <>{cleanedText}</>;
  
  // Find the start of the last sentence (after previous period or newline)
  let start = cleanedText.lastIndexOf('.', lastQ);
  if (start === -1) start = cleanedText.lastIndexOf('\n', lastQ);
  if (start === -1) start = 0; else start += 1;
  
  // Extract question and before/after
  const before = cleanedText.slice(0, start);
  const question = cleanedText.slice(start, lastQ + 1);
  const after = cleanedText.slice(lastQ + 1);
...
**Don't wait ${data.restrictionInfo.daysRemaining} days and lose momentum.**`;
      ...
      // PERSISTENT CHAT HISTORY: ALWAYS attempt to load existing messages first
      // This ensures the entire conversation history is restored on refresh/login
      if (data.sessionId) {
        console.log('🔍 Session ID found:', data.sessionId, '- attempting to load persistent chat history');
        try {
          // Fetch all messages for this session from Supabase
          const messagesResponse = await fetch(`/api/messages?sessionId=${data.sessionId}&userId=${user?.id}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
          });
          ...
          if (messagesResponse.ok) {
            ...
            if (messagesData.messages && messagesData.messages.length > 0) {
              console.log(`✅ Loaded ${messagesData.messages.length} persistent messages from Supabase`);
              
              // Map backend messages to local format and clean internal markers
              const existingMessages = messagesData.messages.map((msg: any) => ({
                id: msg.id,
                text: msg.role === 'assistant' ? cleanInternalMarkers(msg.content) : msg.content, // Clean AI messages only
              ...
              console.log('✅ Persistent chat history restored successfully');
              return;
            }
          }
        } catch (messagesError) {
          console.warn('⚠️ Could not load persistent messages, falling back to session data:', messagesError);
        }
      }
      
      // FALLBACK: If no persistent messages found, check for existing messages in session data
      if (data.messages && data.messages.length > 0) {
        console.log(`✅ Found ${data.messages.length} existing messages in session data - loading them`);
        // Map backend messages to local format and clean internal markers
      ...
      // FINAL FALLBACK: If no persistent messages and no session messages, use firstMessage
      if (data.firstMessage) {
        console.log(`✅ ${data.isPremium ? 'Premium' : 'Free'} user - displaying first message from session_first_message (fallback)`);
        const firstMessage: Message = {
          id: 'session-first-message',
          text: cleanInternalMarkers(data.firstMessage), // Clean internal markers from first message
          isUser: false,
          timestamp: new Date()
        };
...
**Don't wait 30 days and lose momentum.**`,
      ...
      // ULTIMATE FALLBACK: If nothing else worked, show a welcome message
      if (!data.firstMessage && !data.sessionComplete && (!data.messages || data.messages.length === 0)) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Add AI response to messages - clean internal markers before displaying
      const aiMessage: Message = {
        id: (Date.now() + 1).toString(),
        text: cleanInternalMarkers(aiResponse), // Clean internal markers from AI response
        isUser: false,
        timestamp: new Date()
      };
...
**Don't wait 30 days and lose momentum.**`,
  ...
  const handleDebugMessages = async () => {
    ...
    try {
      console.log('🔍 Debug: Testing message retrieval for user:', user.id);
      const response = await fetch(`/api/debug-messages?userId=${user.id}`);
      const data = await response.json();
      ...
      if (data.error) {
        ...
        console.log('✅ Debug successful:', data.summary);
        toast.success(`Debug: ${data.summary.totalMessages} messages, ${data.summary.totalSessions} sessions`);
      }
    } catch (error) {
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessages([
          {
            id: Date.now().toString(),
            text: 'Welcome to your new therapy session! How are you feeling today?',
            isUser: false,
            timestamp: new Date()
          }
        ]);
