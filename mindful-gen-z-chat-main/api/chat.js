...
/**
 * COOLDOWN DETECTOR - Detects when the assistant's final cooldown/end message is produced
 * 
 * This function uses regex patterns to detect session completion messages without modifying prompts.
 * The patterns are versioned and easily extensible for future message variations.
 * 
 * @param {string} assistantMessage - The complete assistant message to analyze
 * @returns {boolean} - True if this is a final cooldown/end message
 */
function detectSessionCompletion(assistantMessage) {
  ...
  // VERSIONED PATTERNS: Centralized list of regex patterns that match final session messages
  // These patterns are based on existing AI responses and can be easily extended
  const completionPatterns = [
    // Primary patterns - exact matches for intentional session endings
  ...
  if (hasCompletionPattern) {
    ...
    const matchedPatterns = completionPatterns.filter(pattern => 
      pattern.test(assistantMessage)
    );
    console.log('🔍 Session completion detected by patterns:', matchedPatterns.map(p => p.source));
    return true;
  }
...
/**
 * SESSION COMPLETION UPDATE - Atomically updates session completion status and cooldown timing
 * 
 * This function performs an idempotent update to mark the session as complete and set cooldown timing.
 * It only updates if the session is not already complete, ensuring no double-completion.
 * 
...
async function updateSessionCompletion(sessionId, userId, isPremium) {
  ...
  try {
    ...
    if (!updateResult || updateResult.length === 0) {
      console.log('ℹ️ Session was already complete or update affected no rows (idempotent)');
      return { success: true, alreadyComplete: true };
    }
    
    // Calculate cooldown timing based on the updated timestamp
    const updatedAt = new Date(updateResult[0].updated_at);
    const cooldownUntil = new Date(updatedAt.getTime() + (COOLDOWN_DURATION_MINUTES * 60 * 1000));

    // Persist cooldown_until for enforcement elsewhere
    const { error: cooldownPersistError } = await supabase
      .from('chat_sessions')
      .update({ cooldown_until: cooldownUntil.toISOString() })
      .eq('id', sessionId);
    if (cooldownPersistError) {
      console.error('❌ Failed to persist cooldown_until:', cooldownPersistError);
    }
    
    console.log(`✅ Session ${sessionId} marked as complete successfully`);
    console.log(`✅ Cooldown until: ${cooldownUntil.toISOString()}`);
    console.log(`✅ User ${userId} session completion logged`);
...
Respond now as a therapist:`;
    ...
    if (session && userId) {
      await supabase.from('chat_messages').insert({
        ...
        created_at: new Date().toISOString()
         ...
         // --- SESSION COMPLETION DETECTION (NEW CENTRALIZED APPROACH) ---
     ...
     try {
       // Use the new centralized detection function
       const isComplete = detectSessionCompletion(aiReply);
       console.log(`🔍 detectSessionCompletion returned: ${isComplete}`);
       ...
       if (isComplete && session?.id) {
         ...
         if (updateResult.success) {
           ...
           if (updateResult.alreadyComplete) {
             console.log('ℹ️ Session was already complete (idempotent)');
           } else {
             console.log('✅ Session completion status updated successfully');
           }
         } else {
           console.error('❌ Failed to update session completion status:', updateResult.error);
    ...
    // Include cooldown information if session is complete
    if (sessionComplete && cooldownUntil) {
      responseData.cooldownUntil = cooldownUntil;
    }
